...
#ifdef ULTIPANEL
static void lcd_z_calibration_menu() 
{
...
static void lcd_move_any(AxisEnum which, float min_pos, float max_pos, const char *hint, menuFunc_t next_menu, menuFunc_t callback=NULL)
{
    if (encoderPosition != 0)
    {
        ...
        plan_buffer_line(delta[X_AXIS], delta[Y_AXIS], delta[Z_AXIS], current_position[E_AXIS], manual_feedrate[which]/60, active_extruder);
        #else
        plan_buffer_line(current_position[X_AXIS], current_position[Y_AXIS], current_position[Z_AXIS], current_position[E_AXIS], manual_feedrate[which]/60, active_extruder);
        #endif
        lcdDrawUpdate = 1;
    }
    if (lcdDrawUpdate)
    {
        lcd_implementation_drawedit(hint, ftostr31(current_position[which]));
    }
    if (LCD_CLICKED)
    {
        lcd_quick_feedback();
